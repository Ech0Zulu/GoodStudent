...
public class VoskSpeechToText : MonoBehaviour
{
	...
	void Start()
	{
		if (AutoStart)
		{
			Log.Instance.WriteLog($"{DateTime.Now:[dd/MM/yy-HH:mm:ss]} : Starting Vosk STT with model: {ModelPath}");
			StartVoskStt();
		}
	}
	...
	private IEnumerator DoStartVoskStt(bool startMicrophone)
	{
		...
		Log.Instance.WriteLog($"{DateTime.Now:[dd/MM/yy-HH:mm:ss]} : Initializing Vosk STT with model: {ModelPath}");
		yield return WaitForMicrophoneInput();
		Log.Instance.WriteLog($"{DateTime.Now:[dd/MM/yy-HH:mm:ss]} : Microphone input initialized");
		...
		Log.Instance.WriteLog($"{DateTime.Now:[dd/MM/yy-HH:mm:ss]} : Model loaded from: {_decompressedModelPath}");
		yield return null;
